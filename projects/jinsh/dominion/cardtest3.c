...
void testCouncilRoomEffect(){
	...
	*tempState = *state;
	...
	if (!cardEffect(council_room, choice1, choice2, choice3, state, handPos, &bonus)
		&& state->handCount[0] == 8)
		printf("councilRoomEffect(): PASS when four cards are drawn\n");
	else
		printf("councilRoomEffect(): FAIL when four cards are drawn\n");
	...
	if (state->hand[0][5] == copper
		...
		printf("councilRoomEffect(): PASS when four cards on the top of deck are added to hands\n");
	else
		printf("councilRoomEffect(): FAIL when four cards on the top of deck are added to hands\n");
		
	// test if num of actions unchanged
	if (state->numActions == tempState->numActions)
		printf("councilRoomEffect(): PASS when num of actions unchanged\n");
	else
		printf("councilRoomEffect(): FAIL when num of actions unchanged\n");

	// test if num of buys increased
	if (state->numBuys == tempState->numBuys + 1)
		printf("councilRoomEffect(): PASS when num of buys increased by one\n");
	else
		printf("councilRoomEffect(): FAIL when num of buys increased by one\n");
	
	// test if council room is discarded
	if (state->hand[0][handPos] != council_room)
		printf("councilRoomEffect(): PASS when council room is discarded\n");
	else
		printf("councilRoomEffect(): FAIL when council room is discarded\n");
	
	// test if discard piles are not shullfed when not needed
	if (state->discardCount[0] != 0)
		printf("councilRoomEffect(): PASS when discard pileis not shuffled when not needed\n");
	else
		printf("councilRoomEffect(): FAIL when discard pileis not shuffled when not needed\n");
	...
	*tempState = *state;
	...
	&& state->discardCount[0] == 0)
		printf("councilRoomEffect(): PASS when discard pile is shuffled when needed\n");
	else
		printf("councilRoomEffect(): FAIL when discard pile is shuffled when needed\n");
	...
	if (state->handCount[1] - tempState->handCount[1] == 1
		...
		printf("councilRoomEffect(): PASS when all other players gain a card\n");
	else
		printf("councilRoomEffect(): FAIL when all other players gain a card]n");
}
