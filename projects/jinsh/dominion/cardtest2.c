...
void testAdventurerEffect(){
	...
	*tempState = *state;
	...
	if (!cardEffect(adventurer, choice1, choice2, choice3, state, handPos, &bonus)
		&& state->handCount[0] == 7)
		printf("adventurerEffect(): PASS when cards are drawn until two Treansures are revealed\n");
	else
		printf("adventurerEffect(): FAIL when cards are drawn until two Treansures are revealed\n");
	...
	if (state->discard[0][0] == estate
		...
		printf("adventurerEffect(): PASS when revealed non-Treausre cards are discarded\n");
	else
		printf("adventurerEffect(): FAIL when revealed non-Treausre cards are discarded\n");
	...
	if (!cardEffect(adventurer, choice1, choice2, choice3, state, handPos, &bonus)
		&& state->deckCount[0] == 0)
		printf("adventurerEffect(): PASS when deck is empty after second Treasure is end of deck\n");
	else	
		printf("adventurerEffect(): FAIL when deck is empty after second Treasure is end of deck\n");
	...
	if (!cardEffect(adventurer, choice1, choice2, choice3, state, handPos, &bonus)
		...
		printf("adventurerEffect(): PASS when cards are shuffled when empty\n");
	else
		printf("adventurerEffect(): FAIL when cards are shuffled when empty\n");
	
	// test if two treasure cards are added to hand
	if (state->hand[0][5] == silver 
		&& state->hand[0][6] == gold)
		printf("adventurerEffect(): PASS when two treausre cards are added to hand\n");
	else
		printf("adventurerEffect(): FAIL when two treasure cards are added to hand\n");
	
	// test if num of actions unchanged
	if (state->numActions == tempState->numActions)
		printf("adventurerEffect(): PASS when num of actions unchaged\n");
	else
		printf("adventurerEffect(): FAIL when num of actions unchaged\n");
	
	// test if num of buys unchanged
	if (state->numBuys == tempState->numBuys)
		printf("adventurerEffect(): PASS when num of buys unchaged\n");
	else
		printf("adventurerEffect(): FAIL when num of buys unchaged\n");
}
